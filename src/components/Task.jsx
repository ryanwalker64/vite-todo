...
function Task({task, completeTask, deleteTask}) {
  return (
    <div className="border-b pb-6 flex">
        <div  
          onClick={completeTask} 
          className={`
          border w-5 h-5 rounded-full border-slate-300 mr-3 text-xs cursor-pointer mt-1
        hover:border-purple-500
          ${task.isComplete && 'bg-purple-500 border-purple-500'}`}>
          </div>
          <div className={task.isComplete ? 'opacity-50' : undefined}>
            <p className="font-medium">{task.title}</p>
            <p className="font-normal text-sm text-slate-400">Tesingsdg dfsdiofnmsf  sdfsf</p>
